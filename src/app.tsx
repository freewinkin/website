...
// ==========================================
// 1. ONBOARDING COMPONENT (Your interactive tool)
// ==========================================
function XvoraOnboarding({ onBack }: { onBack: () => void }) {
  ...
  const handleBackStep = () => {
    if (currentStep > 1) {
      ...
      onBack(); // Takes user back to the landing page
    }
  };
      ...
      <div className="w-full max-w-[1400px] h-[90vh] bg-white rounded-2xl shadow-[0_8px_30px_rgb(0,0,0,0.04)] border border-neutral-100 flex overflow-hidden relative">
        ...
        <div className="flex-1 h-full overflow-y-auto px-16 py-12 flex flex-col justify-between bg-white">
          ...
          <div className="max-w-[850px] w-full mx-auto pt-6 flex items-center justify-between border-t border-neutral-100">
            <button onClick={handleBackStep} className="px-5 py-2.5 border border-neutral-200 rounded-xl text-xs font-semibold text-neutral-700 hover:bg-neutral-50 flex items-center gap-2 bg-white">
              <ChevronLeft className="w-4 h-4" /> Back
            </button>
            <button onClick={handleNext} className="px-6 py-2.5 bg-[#6366F1] hover:bg-[#5558DD] text-white rounded-xl text-xs font-semibold flex items-center gap-2">
...
// ==========================================
// 2. LANDING PAGE COMPONENT (Converted from your HTML)
// ==========================================
  ...
  return (
    <div className="font-sans text-[#0f0f14] bg-white selection:bg-[#ece9fd]">
      <nav className="flex items-center justify-between px-12 py-5 border-b border-[#e6e6ef]">
        ...
        <button onClick={onStart} className="bg-[#403690] text-white px-5 py-2.5 rounded-lg text-sm font-semibold hover:bg-[#332a73] transition-all">
          Sign-In
        </button>
      </nav>
      ...
      <section className="flex items-center gap-10 px-12 py-20 max-w-[1440px] mx-auto flex-wrap">
        <div className="flex-1 min-w-[320px] max-w-[600px]">
          ...
          <div className="flex gap-4">
            <button onClick={onStart} className="bg-[#403690] text-white px-7 py-3.5 font-semibold hover:bg-[#332a73] transition-all shadow-md">
              Analyze My Startup
            </button>
          </div>
        </div>
